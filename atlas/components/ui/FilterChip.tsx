  ...
  (
    ...
    return (
      ...
      >
        ...
        {showClear && (
          // T004 fix: span+role=button instead of <button> — nested <button>
          // is invalid HTML. The outer chip is the focusable element; clear is
          // a click-only inner affordance (Esc + Enter handled by chip).
          <span
            role="button"
            className="ja-filter-chip__clear"
            onClick={(e) => {
              e.stopPropagation();
              onClear?.(e as unknown as React.MouseEvent<HTMLButtonElement>);
            }}
