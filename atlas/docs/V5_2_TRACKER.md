...
| **T103**  | Platform-wide UX consistency + Ramp-grade visual pass (~9)             | P0  | ◐      | `3739cd4` `eb6a474` `abcb8e8` `fb6d841` | T103.11 + T103.1 format helpers + T103.8 two-weight pass + T103.9 page-bg done. T103.2-.7 (approval chip, empty states, labels) + T103.10 (mono charts) pending.                              |
| T103.1    | Format helpers (money/percent/date)                                    | P0  | ✅     | `eb6a474`                               | lib/format/{money,percent,date}.ts — canonical formatters per V5.2 §P5.                                                                                                                       |
| T103.1–.7 | Approval chip, empty states, labels, card padding, tooltips            | P0  | ☐      | —                                       | T103.2 (approval banner→chip), T103.3 (empty-state audit), T103.4 (verb-first labels), T103.5 (card padding), T103.6 (tooltips).                                                              |
| T103.8    | Two-weight system (400/700 only) platform-wide                         | P0  | ✅     | `abcb8e8`                               | Zero fontWeight 500/600 remain across 60+ files. 408 tests.                                                                                                                                   |
| T103.9    | Page bg white + card surfaces                                          | P0  | ◐      | `fb6d841`                               | Page shell = pure white (#fff). Sign-in = sand soft bg. Deeper card-hierarchy sweep (white on grey on white) = remaining work.                                                                |
| T103.10   | Mono charts + lime discipline                                          | P0  | ☐      | —                                       | Charts → monochrome palette; lime only on primary CTA per page.                                                                                                                               |
