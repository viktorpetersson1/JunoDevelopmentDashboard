...
| T093.3     | Per-project tax rate → mig `0024` + Inputs field                       | P0  | ◐      | `e08dc6b`           | Mig 0024 + read-path wired (tax_rate_bps → pct). Inputs edit field pending.                                       |
| T093.7     | Rollout Trigger `lib/finance/rollout-trigger.ts` + mig `0025` globals  | P0  | ◐      | `7d67117` `e08dc6b` | Pure fn (6 tests) + globals mig/wiring done. Summary block + Settings editor pending. ⛔ target/overhead BLOCKED. |
