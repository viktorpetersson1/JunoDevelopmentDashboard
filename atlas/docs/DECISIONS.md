...
| D-025b | Pricing — location factors | 🔲 Open (flagged, not built) | Add `waterfront_type`, `lot_size_acres`, `year_built` (+ maybe `town_proximity`, `view_premium`) to `atlas.projects` + new-project wizard, and thread into the brief + comp-research prompts. Biggest pricing-quality lever: today the AI can't distinguish bayfront from inland, which blows out comp medians. `atlas.comps` already has `waterfront_type`/`lot_size_acres`/`year_built`/`dom_days` columns; projects don't. |
