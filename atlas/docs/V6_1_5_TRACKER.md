...
| **T-PRC-4** | Structured triangulation block for data-gap cases (`sonar-pro`) | P0 | — | ◐ | _(this commit)_ | **◐ Triangulator built + tested; brief-flow integration + UI next.** `TriangulationBlockSchema` (perplexity-schemas) + Zod `TriangulationBlockDataSchema` → `TriangulationBlock`. `triangulator.ts` `runTriangulation(comps + subject + gap severity)` → `sonar-pro` reasoning call → structured block (anchors, adjacent sub-cut, derived band, unresolved questions). Fail-loud (`{ error }`, no block, no Anthropic fallback). Audit `call_site='comp_research'` (V6.1.5-011 — enum has no 'triangulation'; mig budget 0036/0037 only). 3 unit tests. typecheck+lint+test(**571**) clean; golden 23/23. **Next:** fire it in `generateStrategyBrief` when `data_gap_severity != 'none'` + attach `StrategyBrief.triangulationBlock` (additive) + render a collapsible triangulation section (client → build); Big Bing SF (red gap) → 3745 Nassau Point anchor. |
