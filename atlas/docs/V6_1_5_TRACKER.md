...
|------|------|---------|--------|
| **1** | `PERPLEXITY_API_KEY` Cloudflare Pages secret | Set by Viktor in CF dashboard ONLY. Absent locally → adapter + path build behind `BLOCKED-ON-VIKTOR`; Anthropic stays live (flag-off). Live smoke/regression = Viktor-tick. | ⛔ key live in CF unverifiable from here ("A" confirmed by Viktor) |
...
[ ] Framework prompts live in lib/pricing/prompts/*.md
[ ] system-base.md contains the 4 principles + 6 steps + rider/maker thresholds verbatim
[ ] comp-researcher.ts / strategy-brief.ts / location-classifier.ts call callPerplexity only
[ ] Every comp persists source_url, first_listed_at, relist_count, current_dom_days
[ ] pricing_briefs.citations populated as JSONB on every successful run
[ ] Brief + comp table render inline source chips on every number
[ ] Regression passes: Big Bing (SF 1100/1450/1800; inland 650/850/1200), 6 GC (1213-1248/sf), 84 SBR ($7.5M)
