...
| **T-PRC-5** | Buyer-migration thesis via `sonar-reasoning-pro` | P0 | — | ◐ | _(this commit)_ | **◐ Thesis module built + tested; brief-flow integration + downshift + UI next.** `BuyerMigrationThesisSchema` + Zod → `BuyerMigrationThesis`. `buyer-migration-thesis.ts` `runBuyerMigrationThesis` → **`sonar-reasoning-pro`** (the only CoT call), 90s timeout, `call_site='buyer_migration_thesis'`, fail-loud ({ error }, no thesis, no fallback). 4 unit tests (supported, rejected+walkback, error, schema-miss). typecheck+lint+test(**575**) clean; golden 23/23. ⚠ response_format on reasoning-pro unverified live (probe used sonar-pro) — graceful if dirty. **Next:** fire in `generateStrategyBrief` when `data_gap_severity='red'` OR draft `market_maker` → attach `StrategyBrief.buyerMigrationThesis` (persisted via brief jsonb + `pricing_briefs.buyer_migration_thesis` col) + **downshift market_maker→stretch_rider on reject** (presentation gate) + UI section (severity dot; client → build). Big Bing SF → thesis supported, 3745 Nassau anchor. |
