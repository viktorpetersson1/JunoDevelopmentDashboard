...
| **T-PRC-3** | Swap `generateStrategyBrief` → `sonar-pro` + `response_format` + citation persistence + chips + flip | P0 | — | ◐ | `3701084` + _(this commit)_ | **◐ SERVER-SIDE SHIPPED; render chips + location-classifier swap + flip pending.** Foundation (`3701084`): `StrategyBriefSchema` (mirrors `ParsedBriefBody` — V6.1.5-008) + Zod `StrategyBriefBodySchema`. This commit: `generateStrategyBrief` dual-path — when `PRICING_LLM_PROVIDER=perplexity`, `callBriefViaSonar` (sonar-pro, `response_format`=StrategyBriefSchema, **no web search**) → Zod-validate → map → `ParsedBriefBody` → existing compose + `reconcileMath` (untouched). One `runId` per run shared by comp-research + brief audit rows. `recommendation.classification` (rider/maker) threaded — additive, **preserved by reconcileMath** (spreads recommendation). Fail-loud → deterministic fallback brief, no Anthropic retry. `pricing_briefs` persists `citations` (from comp research) + `llm_provider`; `llm_total_cost_usd` → T-PRC-6 telemetry (V6.1.5-009). Route threads citations/provider + comp provenance (closes V6.1.5-006 persistence). Brief regression (4 tests): Big Bing→`market_maker`, 6 GC→`rider`, 84 SBR→`market_rider`, Sonar-fail→fallback. typecheck+lint+test(**565**, +4)+preflight clean; golden 23/23. **Remaining:** citation chips + classification badge on `pricing-strategy-tab.tsx` (client → `npm run build`); `location-classifier.ts` Sonar dual-path (V6.1.5-010); CF flag flip to go live (key already set + validated). |
