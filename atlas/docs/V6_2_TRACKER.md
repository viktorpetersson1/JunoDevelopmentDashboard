...
| **T118** | Capital Sources full ledger + Settings editor                                  | P0  | 0033, 0034  | ☐      | —               | ⛔ Blocked on Viktor: KPC LOC covenants + Harrison Senior terms (or "no Harrison facility"). Schema scaffolds OK; seed-data blocked.            |
| **T119** | Multi-lender support on project Inputs editor + LOC priority UI                | P0  | —           | ☐      | —               | Depends on T118 merge. Native HTML5 DnD (same as T112 pattern).                                                                                  |
| **T120** | Portfolio 36-month cash schedule (aggregator + page)                           | P0  | —           | ☐      | —               | The compute ticket. All Part 2 reads from this. Golden test asserts per-source breakdown sums to `aggregatePortfolio` totals.                    |
