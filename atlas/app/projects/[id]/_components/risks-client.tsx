...
export function RisksClient({
  ...
  async function addRisk() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(json?.error?.message ?? `Failed (HTTP ${res.status})`);
        setSaving(false);
        return;
      }
      setAddOpen(false);
        ...
        >
          ...
          >
            ...
            <Select
              ...
              onChange={(v) => setDraft((d) => ({ ...d, severity: v as DraftRisk['severity'] }))}
            >
            ...
            <Select
              label="Status"
              value={draft.status}
              onChange={(v) => setDraft((d) => ({ ...d, status: v as DraftRisk['status'] }))}
            >
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Button variant="primary" size="sm" onClick={addRisk} loading={saving}>
              Add risk
            </Button>
          </div>
        </div>
              ...
              return (
                <tr key={r.id}>
                  ...
                  <Td>
                    {editing ? (
                      <select
                        value={editDraft.severity ?? r.severity}
                        onChange={(e) =>
                          setEditDraft((d) => ({
                            ...d,
                            severity: e.target.value as DraftRisk['severity'],
                          }))
                        }
                        style={{
                          fontSize: 12,
                      ...
                      <span style={{ fontWeight: 700, color: SEVERITY_COLORS[r.severity] }}>
                        {r.severity}
                      </span>
                    )}
                  </Td>
                  <Td>
                    {editing ? (
                      <textarea
                        value={editDraft.mitigation ?? r.mitigation ?? ''}
                        rows={2}
                        onChange={(e) => {
                          const v = e.target.value || null;
                          setEditDraft((d) => ({ ...d, mitigation: v }));
                        }}
                        style={{
                          width: '100%',
                          padding: '4px 6px',
                    ...
                    {editing ? (
                      <select
                        ...
                        onChange={(e) =>
                          setEditDraft((d) => ({
                            ...d,
                            status: e.target.value as DraftRisk['status'],
                          }))
                        }
                        style={{
                          fontSize: 12,
                    ...
                    <Td>
                      {editing ? (
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          ...
                          <button
                            onClick={() => {
                              if (confirm('Delete this risk?')) void deleteRisk(r.id);
                            }}
                            disabled={saving}
                            style={{
                              fontSize: 11,
