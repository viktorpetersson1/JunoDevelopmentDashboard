...
`.github/workflows/ci.yml` runs on every push to `main` and every PR touching `atlas/**`:
...
| `integration` | Stub. Wired in T076 once a CI-accessible Supabase test branch exists.                        |
| `golden`      | Stub. Wired in T032'–T034' (calc engine port; snapshots against vanilla `public/engine.js`). |
| `build`       | `pnpm build` with placeholder Supabase env vars                                              |

**Required-checks setup (one-time, manual via GitHub UI):**

1. Repo → Settings → Branches → Branch protection rules → `main`.
2. Require status checks: `lint`, `typecheck`, `unit`, `integration`, `golden`, `build`.
3. Require branches to be up-to-date before merging.

The two stub jobs always pass with a deferred-to-ticket message, so they can be marked required from day one without blocking merges; they replace themselves with real assertions when the underlying work lands.
